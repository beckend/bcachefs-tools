...
static int bench_one_parallel_limited(struct bch_fs *, unsigned,
				      void *const *, void *const *, size_t *,
				      unsigned, size_t, unsigned);
			    ...
			    size_t *dst_lens, unsigned nr, size_t chunk_bytes)
{
	...
	for (unsigned i = 0; i < nr; i++) {
		...
		workspace, verify_buf, true, NULL);
		dst_lens[i] = dst_len;
...
int bch2_compress_bench(struct bch_fs *c, unsigned compression_opt)
{
	...
	const unsigned nr = min_t(unsigned, BENCH_NR_CHUNKS,
				  min(nr_workers, BENCH_NR_CHUNKS_MAX));
	...
	ret = bench_one_parallel_limited(c, compression_opt,
					 (void *const *) srcs, dsts, dst_lens,
					 nr, chunk_bytes, nr_workers);
	if (ret)
		goto out;
	...
	pr_info("MT_COMPRESS_BENCH: result opt=0x%x nr_workers=%u chunks=%u chunk_bytes=%u iters=%u serial_ns=%llu parallel_ns=%llu speedup_x100=%u\n",
		compression_opt, nr_workers, nr, chunk_bytes, BENCH_NR_ITERS,
		serial_ns, parallel_ns,
		parallel_ns ? (unsigned)(serial_ns * 100 / parallel_ns) : 0);
	...
	pr_info("MT_COMPRESS_BENCH: PASS\n");
	ret = 0;
...
static int bench_one_parallel_limited(struct bch_fs *c, unsigned compression_opt,
				      void *const *srcs, void *const *dsts,
				      size_t *dst_lens, unsigned nr,
				      size_t chunk_bytes, unsigned eff_workers)
{
	struct bch_compress_wq *cwq = c->compress.mt_wq;
	...
	atomic_t nr_pending;
	atomic_set(&nr_pending, nr);

	for (unsigned i = 0; i < nr; i++) {
		struct bch_compress_worker *w =
			&cwq->workers[i % eff_workers];
		bch2_compress_wq_prepare(&works[i], cwq,
					compression_opt, POS(0, 0),
					srcs[i], chunk_bytes,
					dsts[i], chunk_bytes, w,
					&nr_pending);
	}

	bch2_compress_wq_submit_batch(cwq, works, nr);
	...
	for (unsigned i = 0; i < nr; i++) {
		...
		if (works[i].compression_type ==
			...
			pr_warn("MT_COMPRESS_BENCH_SCALING: parallel chunk %u incompressible\n", i);
			ret = -EINVAL;
			break;
		}
	}
...
int bch2_compress_bench_scaling(struct bch_fs *c, unsigned compression_opt)
{
	struct bch_compress_wq *cwq = c->compress.mt_wq;
	unsigned nr_workers = cwq ? cwq->nr_workers
				  : bch2_compress_nr_workers();
	unsigned chunk_bytes = min_t(unsigned, BENCH_CHUNK_BYTES,
				      c->opts.encoded_extent_max);
	static const unsigned worker_counts[] = { 1, 2, 4, 8, 16, 32 };
	void *srcs[BENCH_NR_CHUNKS_MAX] = {};
	void *dsts[BENCH_NR_CHUNKS_MAX] = {};
	size_t dst_lens[BENCH_NR_CHUNKS_MAX];
	int ret = 0;
	...
	if (!chunk_bytes) {
		pr_err("MT_COMPRESS_BENCH_SCALING: encoded_extent_max is 0; cannot run\n");
		return -EINVAL;
	}

	if (!cwq) {
		pr_err("MT_COMPRESS_BENCH_SCALING: mt_wq is not initialized; cannot run\n");
		return -ENODEV;
	}

	pr_info("MT_COMPRESS_BENCH_SCALING: opt=0x%x nr_wq_workers=%u chunk_bytes=%u iters=%u\n",
		compression_opt, nr_workers, chunk_bytes, BENCH_NR_ITERS);
	pr_info("MT_COMPRESS_BENCH_SCALING: header eff_workers serial_ns parallel_ns speedup_x100 throughput_mib_s_x100\n");

	for (unsigned i = 0; i < BENCH_NR_CHUNKS_MAX; i++) {
		srcs[i] = kvmalloc(chunk_bytes, GFP_KERNEL);
		dsts[i] = kvmalloc(chunk_bytes, GFP_KERNEL);
		if (!srcs[i] || !dsts[i]) {
			ret = -ENOMEM;
	...
	for (unsigned wi = 0; wi < ARRAY_SIZE(worker_counts); wi++) {
		unsigned eff_workers = worker_counts[wi];
		unsigned nr;
		u64 serial_ns = 0, parallel_ns = 0;

		if (eff_workers > nr_workers)
			continue;
...

		{
			u32 speedup_x100 = 0;
			u32 mib_s_x100 = 0;
			u64 total_bytes = (u64)nr * chunk_bytes * BENCH_NR_ITERS;

			if (parallel_ns > 0)
				speedup_x100 = (u32)div64_u64(serial_ns * 100,
								parallel_ns);

			if (parallel_ns > 0)
				mib_s_x100 = (u32)div64_u64(
					total_bytes * 100ULL * NSEC_PER_SEC,
					parallel_ns * 1024ULL * 1024);

			pr_info("MT_COMPRESS_BENCH_SCALING: row %u %llu %llu %u %u\n",
				eff_workers, serial_ns, parallel_ns,
				speedup_x100, mib_s_x100);
		}
	}

	pr_info("MT_COMPRESS_BENCH_SCALING: scaling_done\n");
out:
	for (unsigned i = 0; i < BENCH_NR_CHUNKS_MAX; i++) {
		kvfree(srcs[i]);
		kvfree(dsts[i]);
